...
class FlowMatchScheduler():
    ...
    def add_noise(self, original_samples, noise, timestep):
        if isinstance(timestep, torch.Tensor) and timestep.ndim == 1:
            sigma = torch.stack([
                self.sigmas[torch.argmin((self.timesteps - timestep_item.cpu()).abs())]
                for timestep_item in timestep
            ])
            sigma = sigma.to(device=original_samples.device, dtype=original_samples.dtype)
            sigma = sigma.view(original_samples.shape[0], *([1] * (original_samples.ndim - 1)))
    ...
    def training_weight(self, timestep):
        if isinstance(timestep, torch.Tensor) and timestep.ndim == 1:
            weights = torch.stack([
                self.linear_timesteps_weights[torch.argmin((self.timesteps - timestep_item.to(self.timesteps.device)).abs())]
                for timestep_item in timestep
            ])
        else:
